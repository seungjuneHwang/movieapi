<%@page import="com.test.vo.NBlogVO"%>
<%@page import="com.test.vo.NNewsVO"%>
...
<%
	...
	// search[0] = 라면, search[1] = 고등어
	//search.length 
// 	System.out.print(menu[0]);
// 	System.out.print(menu.length);
	for (String ss : menu) {
		search += ss + "+";
	}
	search += "레시피";
...
// 	}
	...
	try {
		...
		Gson gson = new Gson();
		// 이거 써도 되고
		NBlogVO[] arr = gson.fromJson(json, NBlogVO[].class);
		List<NBlogVO> mcList = Arrays.asList(arr);

		// 이거 써도 되고
		// 			TypeToken<List<MovieVO>> token = new TypeToken<List<MovieVO>>() {};
		// 			List<MovieVO> mcList = gson.fromJson(json, token.getType());

		for (NBlogVO vo : mcList) {
%>
				<a href="<%=vo.getLink() %>"><%=vo.getTitle() %></a>
				<br>
				<%=vo.getDescription() %>
				<br>
			<%}
